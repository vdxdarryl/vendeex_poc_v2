...
const { recordPageView, updateApiUsage, recordSearchEvent, recordCheckoutEvent, recordEngagement, getVisitorCountry } = createAnalytics(db, visitorTracker);
...
async function insertAuditEvent(event) {
  ...
  await db.query(`
    ...
    VALUES ($1, $2, $3, $4, $5, $6)`,
    [event.id || generateId('evt_'), event.eventType, event.userId, event.avatarId || null, JSON.stringify(event.details || {}), event.actor || 'SYSTEM']
  );
}
...
// Phase B: Qdrant vector store — initialise at startup
const qdrant = require('./services/QdrantService');

app.use('/api/auth',  authRoutes({ authService, recordEngagement, visitorTracker, insertAuditEvent, generateSessionToken, generateId }));
app.use('/api/user',  userRoutes({ authService, db, recordEngagement }));
app.use('/api',       searchRoutes({ messageBus, RUN_SEARCH, authService, recordSearchEvent, recordEngagement, getVisitorCountry, multiProviderSearch, AFFILIATE_COM_API_KEY, CHANNEL3_API_KEY }));
app.use('/api',       channel3Routes({ CHANNEL3_API_KEY, CHANNEL3_BASE_URL, AFFILIATE_COM_API_KEY, AFFILIATE_COM_BASE_URL }));
app.use('/api/acp',   checkoutRoutes({ getTaxInfoForLocation, calculateCheckoutShipping, recordCheckoutEvent }));
...
async function startServer() {
  ...
  // Phase B: Ensure Qdrant collections exist (non-blocking — logs warning if unavailable)
  qdrant.ensureCollections().catch(err => {
    console.warn('[Startup] Qdrant collection setup failed (non-fatal):', err.message);
  });
  ...
  app.listen(PORT, () => {
    ...
    console.log(`🗄️  Qdrant: ${process.env.QDRANT_URL ? process.env.QDRANT_URL : 'not configured'}`);
